...
import { Key, Copy, CheckCircle, AlertTriangle, RefreshCw } from 'lucide-react'
...
export function EditApiKeyModal({ isOpen, onClose, onSuccess, accessToken, apiKey }: EditApiKeyModalProps) {
  ...
  useEffect(() => {
    if (apiKey) {
      setName(apiKey.name)
      setStatus(apiKey.status as 'ACTIVE' | 'REVOKED')
    }
  }, [apiKey])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await api.updateApiKey(accessToken, apiKey!.name, { name, status })
  ...
  const handleRegenerateToken = async () => {
    ...
    try {
      const response = await api.updateApiKey(accessToken, apiKey!.name, { generateToken: true })
      if (response.token) {
      ...
      <DialogContent className="sm:max-w-md bg-background border-white/10 text-foreground shadow-2xl shadow-primary/10">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl font-serif font-bold tracking-tight">
            {generatedToken ? <CheckCircle className="h-5 w-5 text-emerald-500" /> : <Key className="h-5 w-5 text-primary" />}
            {generatedToken ? 'Novo Token Gerado!' : 'Editar API Key'}
          </DialogTitle>
        </DialogHeader>
        ...
        {generatedToken ? (
          <div className="space-y-6 pt-4">
            ...
            <div className="space-y-3">
              <Label className="text-[10px] font-bold uppercase tracking-widest text-foreground/40">Novo Token de API</Label>
              <div className="flex gap-2">
                <Input value={generatedToken} readOnly className="font-mono text-sm bg-white/5 border-white/10 selection:bg-primary/30" />
          ...
          <form onSubmit={handleSubmit} className="space-y-6 pt-4">
            <div className="space-y-3">
              <Label htmlFor="edit-name" className="text-[10px] font-bold uppercase tracking-widest text-foreground/40">Nome da Chave</Label>
              <Input
                id="edit-name"
            ...
            <div className="space-y-3">
              <Label htmlFor="edit-status" className="text-[10px] font-bold uppercase tracking-widest text-foreground/40">Status</Label>
            ...
            <div className="space-y-3">
              <Label className="text-[10px] font-bold uppercase tracking-widest text-foreground/40">Gerar Novo Token</Label>
              <Button
                type="button"
                variant="outline"
                onClick={handleRegenerateToken}
                disabled={regeneratingToken}
                className="w-full border-white/10 hover:bg-white/5 font-bold uppercase tracking-widest text-xs h-11"
              >
